            ...
            for (const subOption of option.options) {
                ...
                for (const choice of subOption.choices) {
                    ...
                    if (
                        ...
                        validationErrors.push(
                            `Command ${cmd.name} subcommand ${option.name} option ${subOption.name} choice ${choice.name} has value longer than 100 chars: "${choice.name}" (${choice.name.length} chars)`
                        );
                    }
                }
            }
...
function prepareCommandsForRegistration(commands) {
    ...
    if (removedCommands.length > 0) {
        logger.warn(
            `Commands not registered because of Discord's 100-command limit: ${removedCommands
