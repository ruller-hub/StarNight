...
export async function playQuery(client, interaction, query) {
    const { player, guildData } = await ensurePlayer(
        client,
        interaction,
    );
    ...
    if (!query || !query.trim()) {
        throw new TitanBotError(
            'Invalid query',
            ErrorTypes.USER_INPUT,
            'Please provide a song name or a supported URL.',
        );
    }
    ...
    throw new TitanBotError(
        'No results',
        ErrorTypes.USER_INPUT,
        `No results found. (loadType: ${loadType})`,
    );
}
...
export async function skipTrack(client, interaction) {
    ...
    const title =
        player.current.info?.title || 'Unknown';
